...
<template>
  ...
  <VForm @submit.prevent="handleAccept">
    <VRow>
      ...
      <VCol cols="12" md="4">
        <AppDateTimePicker v-model="teleDate" label="Date of teleconsultation" placeholder="Select Date" />
      </VCol>
      <VCol cols="12" md="4">
        <VSelect class="mt-7" v-model="duration" :items="durations" label="Duration" />
      </VCol>
      <VCol cols="12" md="4">
        <AppDateTimePicker v-model="time" label="Time" type="time" placeholder="Select Time"
    ...
    <VRow>
      <VCol cols="12" class="d-flex justify-end gap-4">
        ...
        <VBtn type="submit" color="success" :disabled="isSubmitting || isAccepted">Accept</VBtn>
      </VCol>
    </VRow>
  </VForm>
